...
curl -s -X PUT \
  ...
  -d "{\"message\":\"Fix data path\",\"content\":\"$CONTENT\",\"sha\":\"$SHA\"}" 2>/dev/null | \
  python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('content',{}).get('name','error'))"
...
curl -s -X PUT \
  ...
  -H "Accept: application/vnd.github.v3+json" \
  "https://api.github.com/repos/$REPO/contents/$FILE" \
  -d "{\"message\":\"Add status data to root\",\"content\":\"$CONTENT\",\"sha\":\"$SHA\"}" 2>/dev/null | \
  python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('content',{}).get('name','error'))"
