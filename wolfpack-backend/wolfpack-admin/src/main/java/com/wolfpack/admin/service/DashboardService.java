...
public class DashboardService {
    ...
    private void initTasks() {
        ...
        TaskVO t5 = new TaskVO();
        t5.setId("task-005");
        t5.setTime("10:00");
        t5.setAssignee("狼牙02");
        t5.setTask("分析昨日直播数据（含行为洞察）");
        t5.setPriority(TaskPriority.HIGH);
        t5.setStatus(TaskStatus.PENDING);
        taskCache.add(t5);
        
        TaskVO t6 = new TaskVO();
        t6.setId("task-006");
        t6.setTime("11:00");
        t6.setAssignee("狼牙03");
        t6.setTask("生成数据看板（含视觉重点）");
    ...
    public List<AgentVO> getAgentList() {
        ...
        // 狼头 - 检查是否有进行中任务
        boolean leaderBusy = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼头"))
        ...
        leader.setStatus(leaderBusy ? AgentStatus.BUSY : AgentStatus.ONLINE);
        leader.setStatusText(leaderBusy ? "在线 - 统筹管理中" : "在线 - 正常运行");
        leader.setTasks(List.of());
        agents.add(leader);
        
        // 狼牙01
        boolean a1Busy = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙01") && t.getTime().compareTo(now.format(DateTimeFormatter.ofPattern("HH:mm"))) <= 0)
            .anyMatch(t -> t.getStatus() == TaskStatus.IN_PROGRESS);
        boolean a1Pending = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙01"))
        ...
        a1.setStatus(a1Busy ? AgentStatus.BUSY : AgentStatus.ONLINE);
        a1.setStatusText(a1Busy ? "在线 - 日报生成中" : a1Pending ? "待命 - 等待明日任务" : "在线 - 空闲");
        a1.setTasks(List.of());
        agents.add(a1);
        
        // 狼牙02
        boolean a2Busy = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙02") && t.getTime().compareTo(now.format(DateTimeFormatter.ofPattern("HH:mm"))) <= 0)
            .anyMatch(t -> t.getStatus() == TaskStatus.IN_PROGRESS);
        boolean a2Pending = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙02"))
            .anyMatch(t -> t.getStatus() == TaskStatus.PENDING);
        ...
        a2.setStatus(a2Busy ? AgentStatus.BUSY : AgentStatus.ONLINE);
        a2.setStatusText(a2Busy ? "在线 - 数据分析中" : a2Pending ? "待命 - 等待明日任务" : "在线 - 空闲");
        a2.setTasks(List.of());
        agents.add(a2);
        
        // 狼牙03
        boolean a3Busy = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙03") && t.getTime().compareTo(now.format(DateTimeFormatter.ofPattern("HH:mm"))) <= 0)
            .anyMatch(t -> t.getStatus() == TaskStatus.IN_PROGRESS);
        boolean a3Pending = taskCache.stream()
            .filter(t -> t.getAssignee().equals("狼牙03"))
            .anyMatch(t -> t.getStatus() == TaskStatus.PENDING);
        ...
        a3.setStatus(a3Busy ? AgentStatus.BUSY : AgentStatus.ONLINE);
        a3.setStatusText(a3Busy ? "在线 - 看板生成中" : a3Pending ? "待命 - 等待明日任务" : "在线 - 空闲");
        a3.setTasks(List.of());
        agents.add(a3);
    ...
    private DashboardDTO.SystemStatus getSystemStatus() {
        ...
        status.setUptime(calculateUptime());
