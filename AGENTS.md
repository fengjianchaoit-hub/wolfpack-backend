...
当用户输入 `/sync` 指令时，触发飞书同步功能：

**指令格式：**
```
/sync          # 同步最新对话记录到飞书
/sync force    # 强制同步全部对话记录
/sync test     # 测试飞书连接
```

**执行步骤：**
1. 读取 `/root/.openclaw/workspace/feishu_sync_config.json` 配置
2. 检查 `webhook_url` 是否已配置
3. 调用 `sync_to_feishu.py` 执行同步
4. 返回同步结果给用户

**注意事项：**
- 需要先在配置文件中设置 `webhook_url`
- 同步功能默认每10分钟自动执行一次
- 失败的消息会加入队列，稍后自动重试

**相关文件：**
- 配置：`/root/.openclaw/workspace/feishu_sync_config.json`
- 脚本：`/root/.openclaw/workspace/sync_to_feishu.py`
- 日志：`/root/.openclaw/workspace/logs/feishu_sync_cron.log`
